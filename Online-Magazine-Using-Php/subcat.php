...
$subcatn = mysql_fetch_array(mysql_query("SELECT id, catid, name FROM subcat WHERE cnm = '$cnm'"));

$newsdtls = mysql_fetch_array(mysql_query("SELECT id, mnm FROM cats WHERE id='".$subcatn[1]."'"));
...
$titleOfMe = " $subcatn[2] - $newsdtls[1] - $basetitle";
?>
...
$ttl = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM news WHERE hide='0' AND  subcat='".$subcatn[0]."'"));
...
<div class="row">
 ...
 <?php
$i = 1;
$ddaa = mysql_query("SELECT id, ttl, thumb, btext, tm FROM news WHERE hide='0' AND subcat='".$subcatn[0]."' ORDER BY id DESC LIMIT ".$start.",".$newsPerPage[0]."");
    echo mysql_error();
...
echo "<li><a href=\"$baseurl/subcat/$cnm/$pSt\"> $pSt</a> </li> ";
}
...
$nextnum=$page+1;
$next ="<li> <a href=\"$baseurl/subcat/$cnm/$nextnum\">&gt;&gt;</a> </li> ";
if($page>=$tpg){
